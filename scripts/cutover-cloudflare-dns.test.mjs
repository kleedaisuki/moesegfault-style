...
test("deletes only the expected GitHub Pages CNAME", async () => {
  ...
  assert.equal(
    await removeLegacyCname({ token: "token", fetchImpl: stub.fetch, log() {} }),
    "deleted",
  );
  assert.equal(stub.calls[2].method, "DELETE");
  assert.match(stub.calls[2].url, /zones\/zone-id\/dns_records\/record-id$/);
});

test("is idempotent after the legacy record is absent", async () => {
  const stub = api([{ result: [{ id: "zone-id" }] }, { result: [] }]);
  assert.equal(
    await removeLegacyCname({ token: "token", fetchImpl: stub.fetch, log() {} }),
    "absent",
  );
...
test("refuses an unexpected DNS target", async () => {
  const stub = api([
    { result: [{ id: "zone-id" }] },
    { result: [{ id: "record-id", content: "unexpected.example" }] },
  ]);
  await assert.rejects(
    removeLegacyCname({ token: "token", fetchImpl: stub.fetch, log() {} }),
    /Refusing to delete/,
  );
  assert.equal(stub.calls.length, 2);
});
...
test("surfaces Cloudflare API failures without exposing the token", async () => {
  ...
  await assert.rejects(
    removeLegacyCname({ token: "secret-token", fetchImpl: stub.fetch, log() {} }),
    (error) =>
...
test("restores the saved CNAME only when the hostname is empty", async () => {
  ...
  assert.equal(
    await restoreLegacyCname({ token: "token", record, fetchImpl: stub.fetch, log() {} }),
    "restored",
  );
  assert.equal(stub.calls[2].method, "POST");
});

test("does not overwrite replacement DNS during rollback", async () => {
  const stub = api([
    { result: [{ id: "zone-id" }] },
    { result: [{ id: "worker-record", type: "AAAA" }] },
  ]);
  assert.equal(
