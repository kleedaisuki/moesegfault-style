 ...
 * @param {{token:string,zoneId:string,fetchImpl?:typeof fetch,log?:(message:string)=>void,beforeDelete?:(record:object)=>Promise<void>}} options Options / 选项。
...
export async function removeLegacyCname({
  ...
  const records = await cloudflare(
    token,
    `/zones/${zoneId}/dns_records?${new URLSearchParams({ type: "CNAME", name: HOSTNAME })}`,
    {},
    fetchImpl,
  );
 ...
 * @param {{token:string,zoneId:string,record:object,fetchImpl?:typeof fetch,log?:(message:string)=>void}} options Options / 选项。
 * @returns {Promise<"restored"|"present">} Recovery result / 恢复结果。
 */
export async function restoreLegacyCname({
  token,
  zoneId,
  record,
  fetchImpl = fetch,
  ...
  const current = await cloudflare(
    token,
    `/zones/${zoneId}/dns_records?${new URLSearchParams({ name: HOSTNAME })}`,
    {},
    fetchImpl,
  );
...
async function main() {
  ...
  if ((process.argv[2] ?? "remove") === "restore") {
    try {
      ...
      await restoreLegacyCname({ token, zoneId, record });
    } catch (error) {
